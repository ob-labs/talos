...
describe('ClaudeExecutor', () => {
  ...
  describe('cleanEnvironment', () => {
    ...
    it('should remove CLAUDE_CODE_ENTRYPOINT environment variable', () => {
      ...
      expect(env.CLAUDE_CODE_ENTRYPOINT).toBe('/path/to/claude');
    });
  ...
  describe('execute', () => {
    it('should return ToolExecutionResult structure', async () => {
      ...
      // The actual execution will fail without claude CLI, but we can verify structure
  ...
  describe('command building scenarios', () => {
    it('should handle basic command (no debug, no model)', () => {
      ...
      // Basic args: --dangerously-skip-permissions --print
      expect(executor.name).toBe('claude');
    });
    ...
    it('should handle debug mode', () => {
      ...
      // With debug: --dangerously-skip-permissions --print --output-format stream-json --verbose
      expect(request.debug).toBe(true);
    });
    ...
    it('should handle model parameter', () => {
      ...
      // With model: --dangerously-skip-permissions --print --model claude-3-5-opus-20241022
      expect(request.model).toBe('claude-3-5-opus-20241022');
    });
