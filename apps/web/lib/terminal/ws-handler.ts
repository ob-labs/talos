...
import { TerminalSessionManager, type PTYSession } from "@talos/terminal";
...
export class TerminalWSServer {
    ...
    if (options.server) {
      ...
      options.server.on("upgrade", (request, socket, head) => {
        ...
        if (pathname === TERMINAL_WS_PATH) {
          ...
          console.log('[TerminalWSServer] Path not matched, destroying socket');
          socket.destroy();
        }
      });
  ...
  private handleConnection(socket: WebSocket, requestUrl?: string): void {
    ...
    socket.on("message", (data: Buffer) => {
      ...
      // PTY creation failed - send error message to client and close socket
      const errorMessage = (error as Error).message || "Failed to create terminal session";
      console.error(`[Terminal] Failed to create PTY session:`, errorMessage);

      if (socket.readyState === WebSocket.OPEN) {
        socket.send(JSON.stringify({
          type: "error",
          message: `Terminal initialization failed: ${errorMessage}. Please check if node-pty is properly installed.`
        }));
   ...
   * 处理来自客户端的消息
   */
  private handleMessage(session: PTYSession, message: WSMessage): void {
    switch (message.type) {
      case "input":
