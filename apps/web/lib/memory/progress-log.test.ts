...
describe("ProgressLogger", () => {
  ...
  async function writeTestContent(content: string) {
    const fullPath = path.join(tempDir, TEST_FILE);
  ...
  describe("parse", () => {
    it("should return empty result for non-existent file", async () => {
      const result = await testLogger.parse();
...
`;
  ...
  describe("appendEntry", () => {
    it("should create new file with entry if file doesn't exist", async () => {
      await testLogger.appendEntry("US-001", ["Created feature"], ["Learning 1"]);
...
`;
      ...
      await testLogger.appendEntry("US-001", ["New feature"], ["New learning"]);
    ...
    it("should format entry with timestamp", async () => {
      await testLogger.appendEntry("US-001", ["Change 1"], []);
    ...
    it("should handle multiline learning text", async () => {
      await testLogger.appendEntry("US-001", ["Change 1"], ["Line 1\nLine 2"]);
...
`;
      ...
      const context = await testLogger.getContext(2);
    ...
    it("should return empty context for non-existent file", async () => {
      const context = await testLogger.getContext();
...
`;
      ...
      const context = await testLogger.getContext(1);
  ...
  describe("addPattern", () => {
    it("should create new file with patterns section if file doesn't exist", async () => {
      await testLogger.addPattern("New pattern");
...
## Execution History`;
      ...
      await expect(testLogger.addPattern("New pattern")).rejects.toThrow(ProgressLogError);
    });
  });
...
`;
      ...
      const entries = await testLogger.getStoryEntries("US-001");
...
`;
      ...
      const entries = await testLogger.getStoryEntries("US-999");
...
`;
      ...
      await testLogger.addCommit("US-001", "abc123def456789");
...
`;
      ...
      await testLogger.addCommit("US-001", "abc123def456789xyz");
...
`;
    ...
    it("should handle multiple commits for the same story", async () => {
      const content = `## Execution History
...
`;
      ...
      await testLogger.addCommit("US-001", "abc123def456789");
      await testLogger.addCommit("US-001", "xyz987ghi654321");

      const updatedContent = await readTestContent();
      expect(updatedContent).toContain("- **commit**: abc123d");
      expect(updatedContent).toContain("- **commit**: xyz987g");

      // Verify both commits are present and in order
      const lines = updatedContent.split("\n");
      const firstCommitIndex = lines.findIndex(line => line.includes("- **commit**: abc123d"));
      const secondCommitIndex = lines.findIndex(line => line.includes("- **commit**: xyz987g"));

      expect(firstCommitIndex).toBeGreaterThan(-1);
      expect(secondCommitIndex).toBeGreaterThan(-1);
      expect(secondCommitIndex).toBeGreaterThan(firstCommitIndex);
    });

    it("should find the most recent entry when multiple entries exist for the same story", async () => {
      const content = `## Execution History
...
`;
      ...
      const firstAttemptIndex = lines.findIndex(line => line.includes("## 2026-02-15 - US-001"));
      const secondAttemptIndex = lines.findIndex(line => line.includes("## 2026-02-16 - US-001"));
      const commitIndex = lines.findIndex(line => line.includes("- **commit**: abc123d"));

      // Commit should be added to the most recent entry
      expect(commitIndex).toBeGreaterThan(secondAttemptIndex);
    });

    it("should throw error if no entry found for story", async () => {
      const content = `## Execution History
...
`;
      ...
      await expect(testLogger.addCommit("US-999", "abc123def456789")).rejects.toThrow(ProgressLogError);
      await expect(testLogger.addCommit("US-999", "abc123def456789")).rejects.toThrow("No entry found for story US-999");
    });

    it("should throw error if progress log doesn't exist", async () => {
      await expect(testLogger.addCommit("US-001", "abc123def456789")).rejects.toThrow(ProgressLogError);
      await expect(testLogger.addCommit("US-001", "abc123def456789")).rejects.toThrow("No progress log found");
    });

    it("should handle entry without proper ending", async () => {
      const content = `## Execution History
...
`;
      ...
      const updatedContent = await readTestContent();
      expect(updatedContent).toContain("- **commit**: abc123d");
      expect(updatedContent).toContain("---"); // Should add proper ending
    });
  });
...
`;
      ...
      const latest = await testLogger.getLatestEntry();
...
`;
      ...
      const results = await testLogger.searchEntries("TypeScript");
...
`;
      ...
      const results = await testLogger.searchEntries("strict mode");
      expect(results).toHaveLength(1);
    });
...
`;
      ...
      const results = await testLogger.searchEntries("nonexistent");
  ...
  describe("getCodebasePatterns", () => {
    it("should return patterns from parsed log", async () => {
      ...
      const patterns = testLogger.getCodebasePatterns(parsed);
...
`;
      ...
      const result = await testLogger.getCommitsByStory();
...
`;
      ...
      const result = await testLogger.getCommitsByStory("US-001");
      expect(result).toHaveLength(1);
      expect(result[0].storyId).toBe("US-001");
      expect(result[0].commits).toEqual(["abc123d"]);
    });

    it("should return empty array when filtering by non-existent storyId", async () => {
      const content = `## Execution History
...
`;
      ...
      const result = await testLogger.getCommitsByStory("US-999");
      expect(result).toEqual([]);
    });

    it("should merge commits from multiple entries for the same story", async () => {
      const content = `## Execution History
...
`;
      ...
      expect(result).toHaveLength(1);
      expect(result[0].storyId).toBe("US-001");
      expect(result[0].commits).toEqual(["abc123d", "def456e"]);
    });

    it("should handle commits in learnings section (should not extract)", async () => {
      const content = `## Execution History
...
- **commit**: abc123d
**Learnings for future iterations:**
- Pattern: Always use commit hashes
---
`;
      await writeTestContent(content);
    ...
    it("should handle case insensitive commit markers", async () => {
      const content = `## Execution History
...
`;
    ...
    it("should handle entries without commits", async () => {
      const content = `## Execution History
...
`;
      ...
      const result = await testLogger.getCommitsByStory();
      expect(result).toHaveLength(1);
      expect(result[0].storyId).toBe("US-002");
      expect(result[0].commits).toEqual(["abc123d"]);
    });
  });
...
`;
      ...
      const result = await testLogger.getStoryProgressEntry("test-prd", "US-999");
...
`;
      ...
      const result = await testLogger.getStoryProgressEntry("test-prd", "US-001");
...
`;
    ...
    it("should return most recent entry if multiple exist for same story", async () => {
      const content = `## Execution History
...
- apps/web/lib/new.ts
**Learnings for future iterations:**
- Learning: Second attempt worked
---
`;
      await writeTestContent(content);
      ...
      expect(result).not.toBeNull();
      expect(result!.implemented).toContain("Second attempt (fixed)");
      expect(result!.filesChanged).toContain("apps/web/lib/new.ts");
      expect(result!.learnings).toContain("Learning: Second attempt worked");
    });
...
`;
      ...
      // Note: The parseChanges method should extract file paths from these patterns
      // Let's verify it correctly identifies the files
...
`;
    ...
    it("should distinguish implementation notes from file paths", async () => {
      const content = `## Execution History
...
- Manual verification: Verified in browser
**Learnings for future iterations:**
- Pattern: Use async/await for database calls
---
`;
      await writeTestContent(content);
...
describe("progressLogger default export", () => {
  it("should export default instance", () => {
    expect(progressLogger).toBeInstanceOf(ProgressLogger);
